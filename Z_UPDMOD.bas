...
'Ёкспорт модулей кода
Private Sub ExportModulesAndClasses()
    Dim vbComp As Object, sFolder$
    
    cntrow = 0  '—брос счетчика колличества строк
    
    ' аталог дл€ экспорта, проверка/создание
    sFolder = ThisWorkbook.Path & "\Macro"
    If Dir(sFolder, vbDirectory) = "" Then CreateDir sFolder
        
    For Each vbComp In ThisWorkbook.VBProject.VBComponents
        Select Case vbComp.Type 'Ёкпорт стандартных модулей, классов, форм и код в Ёта нига
            Case Is = 1, 2, 3, (100 And vbComp.Name = "Ёта нига")
                ExportCodeModule vbComp, sFolder
        End Select
    Next

    Debug.Print "ќбщее количество выгруженных строк кода в данной книге: " & cntrow
...
'Ёкспорт кода из модул€ Ёта нига в текстовый файл sFile
Private Sub ExportCodeToTXT(ByRef vbComp As Object, ByRef sFolder$)
    Dim q&, cnt_line&, Line

    With vbComp.CodeModule
        cnt_line = .CountOfLines
        Open sFolder & "\Ёта нига.txt" For Output As 1
            For q = 1 To cnt_line
                 Print #1, .Lines(q, 1)
            Next
        Close #1
...
'Ћогирование процесса, суммирование общего кол-во строк кода
